...
備註: `Thread.interrupted()`會中斷目前的執行序, 這是一個static方法. 在呼叫完畢後會清除這個執行序的中斷的旗幟(flag)
`Thread.currentThread().isInterrupted()`也會中斷目前的執行序, 但它是實例方法.它不會清除這個執行序的中斷的旗幟(flag). 在這裡兩者都可以用來確認執行序是否有被中斷.
